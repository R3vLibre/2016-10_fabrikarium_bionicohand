...
Elle s'attarderas également sur les différences et difficultés
particulières rencontrées lors du montage des deux mains au
fabrikarium. Ces différences ont deux causes :
...
/Fabrikarium/ seront l'objets des chapitres suivants.
...
*** les vis
Le premier problème rencontré par les deux équipes et la casse
d'entretoises en plastiques et de pas de vis. Il s'agit sans doute
dûes à un problème de commande : les vis utilisées ont un diamètre de
2.2mm au lieu de 2mm.  De plus, il est précisé dans la documentation
en anglais qu'il s'agit de vis auto-foreuse ("tapping screw") ce qui
...
nécessaire d'agrandir les trous de vis. Cet agrandissement se fera
...
- Le trou est ensuite agrandit puis chanfreiné du côté ou on introduit
  l'entretoise
#+attr_html: :width 70% :align center
...
- L'entretoise est ensuite insérée à la presse, dans l'étau de la
  perceuse à colonne.
...
font 16mm au lieu de 15mm comme précisés dans la BOM. Il est
nécessaire de les limer. À voir si des axes de 15mm sont disponibles
dans le commerce, ou si des axes de 14mm qui seraient disponibles
pourrait suffire.
...
Les petits servomoteurs disponibles (modèle TGY-90S) ne sont pas aux
bonnes côtes : 3 mm de plus dans l'épaisseur (dans l'axe de l'arbre du
servo).  Il a donc fallu adapter leur fixation à la paume et leur
liaison au niveau de l'arbre de sortie.
...
(lexique : on appelle "bras" ou "croix" la petite bielle fixée
...
Une cale formée par deux petits bout d'une croix en plastique de servo
...
modification de la pièce, il a donc fallu ré-usiner le fond de 3mm.
...
carte électronique, ce qui n'empêche pas de fermer la main.
...
* Réalisation et modifications de l'éléctronique
** Alimentation
...
** Circuit imprimé : ajout d'un deuxième capteur myoéléctrique
...
1.6.6) : serialPlotter. Cet utilitaire représente en temps réelle des
valeurs reçu sur le port série avec une échelle verticale qui s'adapte
...
certains nombres de valeurs à suivre (une vingtaine) ce qui n'empêche
...
toujours les vingt dernières valeurs reçues qui sont prise en compte
...
-  la médiane courte correspond à un premier filtrage où tous les
   signaux reçu par le capteur sont prises en compte. Elle permet de
...
Dimensionnement de la résistance du circuit : il faut qu'elle soit
...
- avoir un système le plus fin possible en épaisseur pour raccourcir
...
un système de levier à biellette mais il peut être renforcé et
optimisé.
...
système. Deux plaques ont été imprimé à cet effet pour la fixation
...
- les pattes du cueille pomme en abs sont difficilement
  ...
  on peut envisagé de les réalisé à partir de plaque ou de tube d'un
...
- mettre au point un système de réglage et de blocage qui soit déporté
  sur le côté et pas sur le dessus ou autour. voir schéma. Solution
  dessiné qui n'a pas pu être testée. En plus des parties solidaires
